          ...
          <div className="rounded-3xl border border-primary/20 bg-primary/5 p-8 shadow-[var(--shadow-card)]">
            ...
            <div className="mt-6 space-y-4">
              {programs.map((program) => (
                ...
                >
                  <span className="mt-1 inline-flex h-8 w-8 items-center justify-center rounded-full bg-primary text-primary-foreground">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {programs.map((program) => (
              <Link
                ...
                className="group relative block aspect-[3/4] overflow-hidden rounded-2xl bg-primary shadow-[var(--shadow-card)] transition-transform duration-300 hover:-translate-y-1"
              >
                {program.image ? (
                  <img
                    src={program.image}
                ...
                <div className="absolute inset-x-0 bottom-0 p-5 transition-opacity duration-300 group-hover:opacity-0">
                  <h2 className="text-xl font-bold leading-tight text-primary-foreground drop-shadow">{program.title}</h2>
                  <span className="mt-4 inline-flex w-fit items-center rounded-md border border-primary-foreground bg-transparent px-5 py-2 text-sm font-semibold text-primary-foreground transition group-hover:bg-primary-foreground group-hover:text-primary">
                    Learn More
                  </span>
                </div>
                ...
                <div className="absolute inset-0 flex flex-col justify-center bg-primary/90 p-6 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                  <h2 className="text-xl font-bold leading-tight text-primary-foreground">{program.title}</h2>
                  <p className="mt-3 line-clamp-6 text-sm leading-6 text-primary-foreground/90">{program.description}</p>
                  <span className="mt-5 inline-flex w-fit items-center rounded-md border border-primary-foreground bg-transparent px-5 py-2 text-sm font-semibold text-primary-foreground transition group-hover:bg-primary-foreground group-hover:text-primary">
                    Learn More
                  </span>
                </div>
              </Link>
